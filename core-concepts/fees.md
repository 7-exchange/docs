...
If you swap using a referral link, your cost stays exactly the same. 7.Exchange still collects supported provider affiliate fees to the platform fee wallet, and the referrer becomes eligible for a share of that fee under the referral program rules.
...
7.Exchange charges a 0.1% integrator fee on supported swaps. When you swap using a referral link, your cost stays exactly the same: supported provider affiliate fees are collected by the platform, and the referrer is credited internally for the program share.

By default, referrers become eligible for 50% of the provider-reported affiliate or app fee generated by qualifying referred swaps. Admins create weekly payout batches, and each eligible referrer receives one manual transfer for their total unclaimed balance in that batch.
